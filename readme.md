...
3. **Adding Files** - adding files from the file system to the hyperdrive vault.

We'll go through what these are for and a few of the common usages of each element.

### Storage

Every dat vault has **storage**, this is the required first argument for dat-node. By default, we use [dat-storage](http://github.com/datproject/dat-storage) which stores the secret key in `~/.dat/` and the rest of the ddata in `dir/.dat`. Other common options are:
...
Dat('/my-dir', {key: '<key>', sparse: true}, function (err, dat) {
  ...
  dat.vault.readFile('/cat-locations.txt', function (err, content) {
...
There are many ways to get files imported into an vault! Dat node provides a few basic methods. If you need more advanced imports, you can use the `vault.createWriteStream()` methods directly.
...
Initialize a Dat Vault in `dir`. If there is an existing Dat Vault, the vault will be resumed.
...
```js
opts = {
  key: '<dat-key>', // existing key to create vault with or resume
...
* `dat.key`: key of the dat (this will be set later for non-live vaults)
* `dat.vault`: Hyperdrive vault instance.
* `dat.path`: Path of the Dat Vault
* `dat.live`: `vault.live`
* `dat.writable`: Is the `vault` writable?
* `dat.resumed`: `true` if the vault was resumed from an existing database
* `dat.options`: All options passed to Dat and the other submodules

### Module Interfaces

**`dat-node` provides an easy interface to common Dat modules for the created Dat Vault on the `dat` object provided in the callback:**
...
Import files to your Dat Vault from the directory using [mirror-folder](https://github.com/mafintosh/mirror-folder/).
...
*It's important that the `.dat` folder is not imported because it contains a private key that allows the owner to write to the vault.*
...
`dat.trackStats()` adds a `stats` object to `dat`.  Get general vault stats for the latest version:
