...
var dpackStore = require('@dpack/storage')
 ...
 *   `string` is a directory or file. Directory: `/my-data`, storage will be in `/my-data/.dpack`.
...
function defaultStorage (storage, opts) {
  ...
  try {
    // Store in .dpack with secret in ~/.dpack
    if (fs.statSync(storage).isDirectory()) {
      return dpackStore(storage)
    }
  } catch (e) {
